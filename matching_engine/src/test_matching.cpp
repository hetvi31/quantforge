...
void testLimitOrderMatching() {
    ...
    Order sell_order1{1, "BTCUSDT", OrderSide::SELL, OrderType::LIMIT, 50000.0, 10, 10, 0};
    ...
    Order buy_order1{2, "BTCUSDT", OrderSide::BUY, OrderType::LIMIT, 49900.0, 5, 5, 0};
    ...
    Order buy_order2{3, "BTCUSDT", OrderSide::BUY, OrderType::LIMIT, 50000.0, 12, 12, 0};
...
void testFIFOAndPricePriority() {
    ...
    Order s1{101, "ETHUSDT", OrderSide::SELL, OrderType::LIMIT, 3000.0, 5, 5, 0};
    Order s2{102, "ETHUSDT", OrderSide::SELL, OrderType::LIMIT, 3000.0, 5, 5, 0};
    Order s3{103, "ETHUSDT", OrderSide::SELL, OrderType::LIMIT, 2990.0, 5, 5, 0};
    ...
    Order b1{201, "ETHUSDT", OrderSide::BUY, OrderType::LIMIT, 3010.0, 12, 12, 0};
    auto trades = book.addLimitOrder(b1, reports);

    assert(trades.size() == 3);
    // First match: Order 103 (s3)
    assert(trades[0].sell_order_id == 103 && trades[0].price == 2990.0 && trades[0].quantity == 5);
    // Second match: Order 101 (s1)
    assert(trades[1].sell_order_id == 101 && trades[1].price == 3000.0 && trades[1].quantity == 5);
    // Third match: Order 102 (s2)
    assert(trades[2].sell_order_id == 102 && trades[2].price == 3000.0 && trades[2].quantity == 2);
...
void testOrderCancellation() {
    ...
    Order s1{301, "SOLUSDT", OrderSide::SELL, OrderType::LIMIT, 150.0, 10, 10, 0};
    book.addLimitOrder(s1, reports);
    reports.clear();
    ...
    Order b1{302, "SOLUSDT", OrderSide::BUY, OrderType::LIMIT, 150.0, 10, 10, 0};
    auto trades = book.addLimitOrder(b1, reports);
    assert(trades.empty());
